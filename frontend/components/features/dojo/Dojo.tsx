...
import { Plus, Brain, Clock, CheckCircle2, ExternalLink, Code, RefreshCw, Trash2, Download } from 'lucide-react'
...
export default function Dojo({ githubUsername }: DojoProps) {
    ...
    const fetchDueProblems = async () => {
        try {
            const response = await axios.get(`${API_URL}/leetcode/due?github_username=${githubUsername}`)
    ...
    const handleDeleteProblem = async (problemId: number) => {
        ...
        try {
            await axios.delete(`${API_URL}/leetcode/${problemId}?github_username=${githubUsername}`)
            fetchDueProblems()
        } catch (error) {
    ...
    const handleAddProblem = async (e: React.FormEvent) => {
        e.preventDefault()
        try {
            await axios.post(`${API_URL}/leetcode/log?github_username=${githubUsername}`, newProblem)
    ...
    const handleLogReview = async (problemId: number, rating: number) => {
        try {
            await axios.post(`${API_URL}/leetcode/${problemId}/review-log?github_username=${githubUsername}`, {
    ...
    const handleSeedBlind75 = async () => {
        if (!window.confirm('Import all Blind 75 problems? This will add any missing problems to your SRS deck.')) return

        setLoading(true)
        try {
            await axios.post(`${API_URL}/leetcode/seed/blind75?github_username=${githubUsername}`)
            fetchDueProblems()
            alert("Blind 75 problems imported successfully!")
        } catch (error) {
            console.error("Failed to seed Blind 75", error)
            alert("Failed to import Blind 75")
        } finally {
            setLoading(false)
        }
    }
        ...
        <div className="h-full flex flex-col space-y-6">
            ...
            <div className="flex items-center justify-between bg-[#242424] p-6 rounded-2xl border border-white/10 shadow-lg">
                ...
                <div className="flex space-x-3">
                    <button
                        onClick={handleSeedBlind75}
                        className="flex items-center space-x-2 px-4 py-2 bg-[#242424] hover:bg-[#2a2a2a] text-[#FBFAEE]/80 hover:text-[#FBFAEE] rounded-xl font-medium transition border border-white/10"
                        title="Import Blind 75 Problem Set"
                    >
                        <Download className="w-5 h-5" />
                        <span className="hidden md:inline">Import Blind 75</span>
                    </button>
                    <button
